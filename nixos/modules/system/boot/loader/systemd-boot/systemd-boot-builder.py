...
def write_loader_conf(profile, generation):
    ...
    os.rename(
        "@efiSysMountPoint@/loader/loader.conf.tmp",
        "@efiSysMountPoint@/loader/loader.conf",
    )
...
def write_entry(profile, generation, machine_id):
    ...
    if is_xen_efi:
        ...
        copy_from_profile_to_boot_subpath(
            profile, generation, "kernel", "%s/kernel" % (xen_efi_dir_path,)
        )
        initrd = copy_from_profile_to_boot_subpath(
            profile, generation, "initrd", "%s/initrd" % (xen_efi_dir_path,)
        )
        xen = copy_from_profile_to_boot_subpath(
            profile, generation, "xen.efi", "%s/xen.efi" % (xen_efi_dir_path,)
        )
        copy_if_not_exists(
            xen_cfg_path, "@efiSysMountPoint@%s/xen.cfg" % (xen_efi_dir_path,)
        )
    ...
    try:
        append_initrd_secrets = profile_path(
            profile, generation, "append-initrd-secrets"
        )
        subprocess.check_call(
            [append_initrd_secrets, "@efiSysMountPoint@%s" % (initrd)]
        )
    except FileNotFoundError:
        pass

    if profile:
        entry_file = "@efiSysMountPoint@/loader/entries/nixos-%s-generation-%d.conf" % (
            profile,
            generation,
        )
    else:
        entry_file = "@efiSysMountPoint@/loader/entries/nixos-generation-%d.conf" % (
            generation
        )
    ...
    if not is_xen_efi:
        kernel_params = "systemConfig=%s init=%s/init " % (
            generation_dir,
            generation_dir,
        )
...
def remove_old_entries(gens):
    global known_paths
    rex_profile = re.compile(
        "^@efiSysMountPoint@/loader/entries/nixos-(.*)-generation-.*\.conf$"
    )
    rex_generation = re.compile(
        "^@efiSysMountPoint@/loader/entries/nixos.*-generation-(.*)\.conf$"
    )
    for path in glob.iglob(
        "@efiSysMountPoint@/loader/entries/nixos*-generation-[1-9]*.conf"
    ):
        try:
...
def main():
    ...
    if os.getenv("NIXOS_INSTALL_BOOTLOADER") == "1":
        ...
        if "@canTouchEfiVariables@" == "1":
            subprocess.check_call(
                ["@systemd@/bin/bootctl", "--path=@efiSysMountPoint@", "install"]
            )
        else:
            subprocess.check_call(
                [
        ...
        for path in glob.iglob("@memtest86@/*"):
            if os.path.isdir(path):
                shutil.copytree(
                    path,
                    os.path.join(
                        "@efiSysMountPoint@/efi/memtest86", os.path.basename(path)
                    ),
                )
            else:
